...
@app.route('/streamlit.html')
def serve_streamlit_html():
    return render_template('streamlit.html')

@app.route('/run_streamlit')
def run_streamlit():
    # Start the Streamlit app and capture its output
    output = subprocess.check_output(['streamlit', 'run', 'streamlit_app.py'], universal_newlines=True, stderr=subprocess.STDOUT)
    
    return f"Streamlit Output: {output}"
...
#     # return "Running the Streamlit app..."
